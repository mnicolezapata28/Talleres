...
El lenguaje Python tiene la sintaxis más fácil de comprender, porque es directa y  usa palabras familiares, sin necesidad de conocer conceptos avanzados de programación funcional.
...
```scala
def getAverage(nums: List[Int]): Double =
  nums.map(_.toDouble).sum / nums.size

getAverage(List(1, 2, 3, 4, 5))
